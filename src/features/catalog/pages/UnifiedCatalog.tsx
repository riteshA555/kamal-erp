...
import { GSTSettings, PricingSettings } from '../../../types/settings'
import { getLatestRate } from '../../../services/rateService'
import { Product, JobWorkItem } from '../../../types'
import { Search, Package, Hammer, Edit2, Trash2, Check, X, Plus, AlertCircle, Calculator } from 'lucide-react'
...
export default function UnifiedCatalog() {
    ...
    const loadData = async () => {
        ...
        try {
            const [p, s, gst, pricing, rate] = await Promise.all([
                getProducts(),
                getJobWorkItems(),
            ...
            setSilverRate(rate ? rate.rate_1g : 0)
        } catch (err) {
            console.error(err)
        } finally {
            setLoading(false)
    ...
    const openAddForm = () => {
        ...
        setNewValues({
            ...
            default_rate: '',
            current_stock: '',
            gst_rate: gstSettings?.defaultGstRateSale || '3', // Use Scale setting
            labour_cost: pricingSettings?.defaultMakingCharge || ''
        })
    }
    ...
    const handleUpdateProduct = async () => {
        ...
        try {
            await updateProduct(editingId, {
                ...
                gst_rate: Number(editValues.gst_rate)
            })
            setEditingId(null)
            loadData()
        } catch (err: any) {
        ...
        try {
            if (itemTypeToAdd === 'Products') {
                ...
                await addProduct({
                    ...
                    gst_rate: Number(newValues.gst_rate || 3),
                    is_active: true
                })
            } else {
    ...
    return (
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                <div>
                    <h1 style={{ margin: 0, fontSize: '1.75rem' }}>Business Catalog</h1>
                ...
                <div style={{
                    ...
                    <div className="grid-responsive" style={{ gap: '2rem' }}>
                        ...
                        {itemTypeToAdd === 'Products' ? (
                            <>
                                ...
                                <div>
                                    <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 600, color: '#475569', marginBottom: '0.4rem' }}>GST Rate (%)</label>
                                    <input type="number" step="0.1" placeholder="3.0" style={{ width: '100%', padding: '0.75rem', borderRadius: '10px', border: '1px solid #cbd5e1' }} value={newValues.gst_rate || ''} onChange={e => setNewValues({ ...newValues, gst_rate: e.target.value })} />
                                </div>
                                <div>
                    ...
                    {itemTypeToAdd === 'Products' && (
                        <div style={{ marginTop: '2rem', padding: '1.5rem', background: '#f8fafc', borderRadius: '16px', border: '1px dashed #cbd5e1' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem', color: '#475569', fontWeight: 600 }}>
                                <Calculator size={18} /> Price Estimator (Live)
                            </div>
                            <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap' }}>
                                <div>
                                    <div style={{ fontSize: '0.8rem', color: '#64748b' }}>Silver Value ({silverRate}/g)</div>
                                    <div style={{ fontSize: '1.1rem', fontWeight: 700 }}>₹{((Number(newValues.default_weight) || 0) * silverRate).toLocaleString()}</div>
                                </div>
                                <div>
                                    <div style={{ fontSize: '0.8rem', color: '#64748b' }}>Labour</div>
                                    <div style={{ fontSize: '1.1rem', fontWeight: 700 }}>₹{Number(newValues.labour_cost || 0).toLocaleString()}</div>
                                </div>
                                <div>
                                    <div style={{ fontSize: '0.8rem', color: '#64748b' }}>GST ({newValues.gst_rate}%)</div>
                                    <div style={{ fontSize: '1.1rem', fontWeight: 700 }}>
                                        ₹{(((Number(newValues.default_weight) || 0) * silverRate + (Number(newValues.labour_cost) || 0)) * (Number(newValues.gst_rate) || 3) / 100).toFixed(0)}
                                    </div>
                                </div>
                                <div style={{ borderLeft: '2px solid #cbd5e1', paddingLeft: '1.5rem' }}>
                                    <div style={{ fontSize: '0.8rem', color: '#64748b' }}>Approx Selling Price</div>
                                    <div style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--color-primary)' }}>
                                        ₹{(((Number(newValues.default_weight) || 0) * silverRate + (Number(newValues.labour_cost) || 0)) * (1 + (Number(newValues.gst_rate) || 3) / 100)).toFixed(0)}
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        filteredProducts.map(p => (
                            <div key={p.id} style={{ background: 'white', padding: '1.25rem', borderRadius: '12px', border: editingId === p.id ? '2px solid var(--color-primary)' : '1px solid var(--color-border)', position: 'relative' }}>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', fontSize: '0.85rem' }}>
                                    ...
                                    <div style={{ background: '#f8fafc', padding: '0.4rem', borderRadius: '6px' }}>
                                        GST%: {editingId === p.id ?
                                            <input type="number" style={{ width: '40px', marginLeft: '5px' }} value={editValues.gst_rate} onChange={e => setEditValues({ ...editValues, gst_rate: e.target.value })} /> :
                                            <b>{p.gst_rate || 3}%</b>
                                        }
                                    </div>
                                </div>
