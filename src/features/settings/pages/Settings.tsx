...
import { getSettings, updateSettings, resetSettings, factoryReset } from '../../../services/settingsService'
import {
...
function NotificationSettingsForm({ settings, onChange }: { settings: NotificationSettings, onChange: (s: NotificationSettings) => void }) {
    ...
    return (
        <div className="grid-responsive" style={{ gap: '1.5rem' }}>
            ...
            <FormField label="EmailJS Configuration (For Real Emails)" style={{ gridColumn: '1 / -1', borderTop: '1px solid #e2e8f0', paddingTop: '1rem' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
                    <div>
                        <label style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.85rem', fontWeight: 500 }}>Service ID</label>
                        <input
                            type="text"
                    ...
                    <div>
                        <label style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.85rem', fontWeight: 500 }}>Template ID</label>
                        <input
                            type="text"
                            value={settings.emailJsTemplateId || ''}
                            onChange={e => update('emailJsTemplateId', e.target.value)}
                            placeholder="e.g. template_xxxx"
                            style={{ padding: '0.5rem', fontSize: '0.9rem' }}
                        />
                    </div>
                    <div>
                        <label style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.85rem', fontWeight: 500 }}>Public Key</label>
                        <input
                            type="password"
                ...
                <p style={{ fontSize: '0.75rem', color: '#64748b', marginTop: '0.5rem' }}>
                    Create a free account at <a href="https://www.emailjs.com/" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-primary)' }}>emailjs.com</a> to get these keys. This allows sending real emails directly from the browser.
                    <br /> Note: Ensure your template has variables like <code>{'{{to_name}}'}</code>, <code>{'{{message}}'}</code>, etc.
                </p>
            </FormField>
